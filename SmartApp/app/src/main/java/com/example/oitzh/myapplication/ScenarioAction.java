...
public class ScenarioAction implements Serializable {
    ...
    public static class Action implements Serializable {
        ...
        public void setParameter(Object o) {
            ...
            if (o instanceof Alert) alert = (Alert) o;
        }
        ...
        public enum Alert {
            ...
            public static String[] enumToDescriptionArray() {
                return Arrays.stream(Alert.values()).map(val -> val.getClass().toString().substring(val.getClass().toString().lastIndexOf("$") + 1) + " " + val.name()).toArray(String[]::new);
            }

            public static Alert descriptionToEnum(String str) {
                String trigger = str.split(" ")[1];
                return Arrays.stream(Alert.values()).filter(val -> val.name().contains(trigger)).toArray(Alert[]::new)[0];
            }
        }
    }
}
